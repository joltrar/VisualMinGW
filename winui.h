...
class CFont
{
	...
	HFONT Create(int nHeight, int nWidth, int nEscapement, int nOrientation, 
...
class CIniFile
{
	...
	bool GetSection(char * section);
	bool GetData(char * key, char * data);
	bool FindSection(char * section);
	bool FindData(char * key, char * data);
	bool SkipUnwantedChars(void);
	bool LineFeed(void);
	bool SkipUnwanted(void);
	bool CmpChars(char * string);
};
